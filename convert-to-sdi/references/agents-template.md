...
This is the canonical template the skill uses in Phase 2 to generate the project's `AGENTS.md` and `CLAUDE.md` (or to merge into existing files per the Strategy B procedure in `existing-artifact-handling.md`). The skill copies the same customized content into both files at the project repo root, then the user keeps whichever file(s) their coding agents read.
...
- **Pre-populate `Work tracker`** with a single retroactive one-line row marking the existing project state ("converted to SDI on YYYY-MM-DD; first work item TBD"), and mirror it with a retroactive `## Framework adoption` section in `docs/WORK_LOG.md`.
